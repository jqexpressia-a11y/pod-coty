...
Claude Code is the **implementation agent** in this workflow. Manus is the architecture agent that writes architecture intent, task pods, and acceptance criteria. Claude Code implements requested changes by reading the committed repository state, making code or documentation edits, running relevant checks, and committing the result back to Git so the configured downstream deployment automation can react to the new commit.
