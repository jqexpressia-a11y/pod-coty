...
def main() -> int:
    ...
    payload = {
        ...
        "expected_repository": args.expected_repository or args.repository,
        "branch": args.branch,
        "ref": args.ref,
        "after": args.sha,
    ...
    status, text, parsed = post_json(args.url, body, args.secret, args.timeout)
    print(f"KVM webhook HTTP status: {status}")
    if text:
        print(text)
    if status < 200 or status >= 300:
        print("KVM deployment webhook returned a non-success HTTP status.", file=sys.stderr)
        return 1
    if parsed is None:
        print("KVM deployment webhook returned no parseable JSON response.", file=sys.stderr)
        return 1
    success = parsed.get("success") is True or parsed.get("status") in {"ok", "success", "completed"}
    healthy = parsed.get("healthy", True) is not False
    if not success or not healthy:
        print("KVM deployment webhook did not report a successful healthy deployment.", file=sys.stderr)
        return 1
    return 0
